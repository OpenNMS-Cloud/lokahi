...
public class DiscoveryConfigGrpcService extends DiscoveryConfigOperationGrpc.DiscoveryConfigOperationImplBase {
    ...
    @Override
    public void createConfig(DiscoveryConfigRequest request, StreamObserver<DiscoveryConfigByLocationDTO> responseObserver) {
        tenantLookup.lookupTenantId(Context.current())
            .ifPresentOrElse(tenantId -> {
                try {
                    DiscoveryConfigByLocationDTO discoveryConfigByLocation = configService.createOrUpdateConfig(request, tenantId);
                    responseObserver.onNext(discoveryConfigByLocation);
    ...
    @Override
    public void listDiscoveryConfig(Empty request, StreamObserver<DiscoveryConfigByLocationDTO> responseObserver) {
        tenantLookup.lookupTenantId(Context.current())
            .ifPresentOrElse(tenantId -> configService.listDiscoveryConfigs(tenantId).ifPresentOrElse(discoveryConfigByLocation -> {
                responseObserver.onNext(discoveryConfigByLocation);
                responseObserver.onCompleted();
            }, () -> responseObserver.onError(StatusProto.toStatusRuntimeException(createStatus(Code.NOT_FOUND_VALUE,
                "Can't find discovery config for tenant: " + tenantId)))), () -> responseObserver.onError(StatusProto.toStatusRuntimeException(createMissingTenant())));
    }

    @Override
    public void getDiscoveryConfigByName(StringValue request, StreamObserver<DiscoveryConfigByLocationDTO> responseObserver) {
        tenantLookup.lookupTenantId(Context.current())
            .ifPresentOrElse(tenantId ->

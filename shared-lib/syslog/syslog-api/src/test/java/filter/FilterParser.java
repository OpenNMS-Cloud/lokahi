...
public class FilterParser {
    ...
    private Filter and() {
        ...
        return (Filter) new AndFilter(filters);
    }
    private Filter or() {
        match("|");
        List<Filter> filters = filterList();
        return (Filter) new OrFilter(filters);
    }
    private Filter not() {
        match("!");
    ...
    private Filter eq(String attribute) {
        ...
        if ("*".equals(value.trim())) {
            return (Filter) new PresenceFilter(attribute);
        }

        // a simple equals filter
        if (!value.replace("\\*", "").contains("*")) {
            return (Filter) new EqFilter(attribute, value.replaceAll("\\\\(.)", "$1"));
        }

        return (Filter) new PatternMatchingFilter(attribute, value);
    ...
    private Filter lessThan(String attribute) {
        ...
        return (Filter) new LessThanFilter(attribute, value);
    }
    ...
    private Filter greaterThan(String attribute) {
        ...
        return (Filter) new GreaterThanFilter(attribute, value);
    }

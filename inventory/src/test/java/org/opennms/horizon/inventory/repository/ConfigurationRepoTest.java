...
public class ConfigurationRepoTest {
    ...
    @Test
    void testFindByKey() {
        List<Configuration> result = repository.findByKey(configuration1.getKey());
        assertThat(result.size()).isEqualTo(2);
    }

    @Test
    void testFindByKeyNotExist() {
        List<Configuration> result = repository.findByTenantId("key-not-exist");
        assertThat(result.size()).isZero();
    }

    @Test
    void testFindByTenantIdAndKeyAndLocation() {
        List<Configuration> result = repository.findByTenantIdAndKeyAndLocation(tenantId1, configuration1.getKey(), configuration1.getLocation());
        assertThat(result.size()).isEqualTo(1);
        assertThat(result.get(0).getValue().toString()).isEqualTo("{\"test\":\"value1\"}");
    }

    @Test
    void testFindByLocation() {
        List<Configuration> result = repository.findByLocation(configuration1.getLocation());
        assertThat(result.size()).isEqualTo(3);
    }

    @Test
    void testFindByLocationNotExist() {
        List<Configuration> result = repository.findByLocation("random configuration");
        assertThat(result.size()).isZero();
    }
